...
const double PGenerateGoldenApple = .01;
...
// Receives 3 inputs
// @parameter double for x location
// @parameter double for y location
// @parameter char for what is there in that area (space, apple, snake, etc.)
...
void Generate(double genType, char typePlace)
{
	if (rand() / (double)RAND_MAX < genType) {
		...
		if (board[x][y] != 'S' && board[x][y] != 's') {
			switch (typePlace)
			{
			case 'A': if (board[x][y] != 'G') board[x][y] = 'A'; break;
			case 'G': if (board[x][y] != 'A') board[x][y] = 'G'; break;
			}
		}
	}
}
...
void update() {
	if (!paused)
	{
		...
		Generate(PGenerateFruit, 'A');
		Generate(PGenerateGoldenApple, 'G');
